...
export function CookieBanner() {
  const { showBanner, region, gpcDetected, acceptAll, rejectAll, openCustomize, dismissBanner } = useConsent()
      ...
      {showBanner && (
        <>
          ...
          <motion.div
            ...
            className={cn(
              "fixed inset-x-0 bottom-0 z-50 mx-auto w-full max-w-2xl p-3 sm:p-4",
              "pointer-events-none"
            )}
          >
            <div className="pointer-events-auto overflow-hidden rounded-xl border bg-card shadow-lg ring-1 ring-foreground/5">
              ...
              <div className="flex items-center gap-2 border-b px-4 py-3">
                ...
                <span id="cookie-banner-title" className="text-sm font-medium">Cookie Preferences</span>
                {gpcDetected && (
                  <span className="ml-auto inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2 py-0.5 text-[11px] font-medium text-emerald-700 dark:text-emerald-400">
                    <ShieldCheck aria-hidden="true" className="size-3" /> GPC honored
                  </span>
                )}
              ...
              <div className="border-b px-4 py-3">
                <p id="cookie-banner-desc" className="text-sm leading-relaxed text-muted-foreground">
                  ...
                  {" "}
                  {gpcDetected && (
                    <span className="font-medium text-foreground">Global Privacy Control detected — marketing cookies are off by default. </span>
                  )}
                </p>
                <Link href="/privacy" className="mt-1.5 inline-block cursor-pointer text-xs text-muted-foreground underline underline-offset-2 transition-colors hover:text-foreground">
                  Read our Privacy Policy
                </Link>
                {region === "us_opt_out" && (
                  <Link href="/cookies#do-not-sell" className="ml-3 inline-block cursor-pointer text-xs text-muted-foreground underline underline-offset-2 hover:text-foreground">
              ...
              <div className="px-4 py-3">
                <div className="flex items-center gap-2">
                  <Button size="sm" className="h-8 flex-1 cursor-pointer text-xs" onClick={acceptAll}>
                    Accept All
                  </Button>
                  <Button variant="outline" size="sm" className="h-8 flex-1 cursor-pointer text-xs" onClick={rejectAll}>
                    Reject All
                  </Button>
                  <Button variant="ghost" size="sm" className="h-8 flex-1 cursor-pointer text-xs" onClick={openCustomize}>
                    Customize
                  </Button>
                </div>
              </div>
              ...
              <div className="bg-muted/40 px-4 py-2.5">
                <p className="text-center text-xs text-muted-foreground">GDPR · CCPA · LGPD compliant</p>
              </div>
            </div>
          </motion.div>
        </>
